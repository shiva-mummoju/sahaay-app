...
export class AuthguardService implements CanActivate{
  ...
  constructor(private router: Router,private toastr:ToastrService,private authService: AuthService) { }
    ...
    icango(x:string){
      ...
      if( x == 'proctorreview' || x == 'proctorremark' ||   x=='view' || x=='history' ||    x== 'guestlist' || x=="semesterrejected" || x == 'proctorlist' || x == 'adminreview' || x=='adminremark' || x == 'allaccepted' || x=='semesterall'){
        if(this.currentuserrank == 'admin'){
          return true;
        }
      }
